...
$sql='SELECT id,title,price,package_pl,package_ang FROM product WHERE $_GET[id]';
$sql1="SELECT * FROM product WHERE product.id=$_GET[id]";
$product=$pdo->prepare($sql1);
$product->execute();
        ...
        jQuery(document).ready(function($){
            ...
            function calc(){
                ...
                var kamen=$('.kamni-spisok').val(),
                    shirina=$('.shirina').val(),
                    visota=$('.visota').val(),
                    ucet_uglov=$('.ucet-uglov-input').val() || 0,
                    ...
                    ugli=parseFloat($('.ucet-uglov-input').on('input').val()),
                    fields_ready=0;
                //alert(ugli);
                /* проверка полей */
                switch(true){
                ...
                $('.itogo-pack-ang').html(Math.ceil(ugli/pack_ang) || 0);
...
<table align="center" style="background-image: url("")">
    ...
    <tr>
        <td>Итого упаковок плскостных элементов: <span class="itogo-pack-pl"></span></td>
    </tr>
    <tr>
